...
class PPF_localStorage: NSObject {
    ...
    private override init() {
        ...
        documents = NSSearchPathForDirectoriesInDomains(FileManager.SearchPathDirectory.documentDirectory, FileManager.SearchPathDomainMask.userDomainMask, true).first!
        library = NSSearchPathForDirectoriesInDomains(FileManager.SearchPathDirectory.libraryDirectory, FileManager.SearchPathDomainMask.userDomainMask, true).first!
        
        tmp = NSTemporaryDirectory()
        
        caches = NSSearchPathForDirectoriesInDomains(FileManager.SearchPathDirectory.cachesDirectory, FileManager.SearchPathDomainMask.userDomainMask, true).first!
        preferences = library.appending("/Preferences")
        super.init()
    }
...
extension PPF_localStorage{
    ...
    func createFolderWithPath(path:String,withName name:String) -> Bool {
        ...
        if folderExistsAtPath(path: fullPath){
            return true
        }
        do{
    ...
    func createFolderInDocuments(folderName:String) -> Bool {
        return createFolderWithPath(path: documents, withName: folderName)
    }
    ...
    func createFolderInCaches(folderName:String) -> Bool {
        return createFolderWithPath(path: caches, withName: folderName)
    }
    ...
    func deleteItemInDirectory(path:String) {
        guard let subPath = subpathInDirectory(dir: path) else{
            return
        }
        for s in subPath{
            let s = path.appending("/\(s)")
            deleteFile(path: s)
        }
    }
    ...
    func folderExistsAtPath(path:String) -> Bool {
        ...
        let e = fileManager.fileExists(atPath: path, isDirectory: &isDir)
    ...
    func subpathInDirectory(dir:String) -> [String]? {
        ...
        do{
            let subP = try fileManager.subpathsOfDirectory(atPath: dir)
            return subP
        }catch let error as NSError{
            print(error)
            return nil
...
extension PPF_localStorage{
    ...
    func saveImage(image:UIImage,atPath path:String,type:ImageType,cover:Bool) -> Bool {
        ...
        if cover && fileExistsAtPath(path: path){
            deleteFile(path: path)
        }
        
        
        do{
            try imageD.write(to: url, options: Data.WritingOptions.atomic)
            return true
        }catch{
...
//        return imageD.writeToFile(path, atomically: true)
    ...
    func createImageComletyNameInRootPath(rootPath:String,type:ImageType) -> String{
        ...
        guard createFolderWithPath(path: rootPath, withName: "images") else{
            fatalError("创建文件夹不成功")
        }
...
extension PPF_localStorage{
    ...
    func createImageCompletyNameWithType(type:ImageType)->String {
        return createImageComletyNameInRootPath(rootPath: caches, type: type)
    }
}
